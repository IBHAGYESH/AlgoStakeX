...
const NETWORK_CONFIG = {
  mainnet: {
    ...
    contractWalletAddress: "ESEUVKN4EGRLZHQJPS7AH3ITLQMFG3LABXD4VXZVJGHZEZU2JMEMJRA6NU",
...
function decodeBoxName(boxNameBytes) {
  try {
    ...
    const view = new DataView(boxNameBytes.buffer, boxNameBytes.byteOffset, boxNameBytes.byteLength);
...
function decodeStakeData(boxValueBytes) {
  try {
    ...
    let rewardTypeBytes = boxValueBytes.slice(rewardTypeOffset + 2, rewardTypeOffset + 2 + rewardTypeLength);
    ...
    let utilityBytes = boxValueBytes.slice(utilityOffset + 2, utilityOffset + 2 + utilityLength);
...
class AlgorandService {
  ...
  async getBoxData(boxNameBase64) {
    try {
      ...
      const url = `${this.config.algodUrl}/v2/applications/${this.config.contractApplicationId}/box?name=${encodeURIComponent(encodedName)}`;
    ...
    try {
      ...
      for (const box of allBoxes) {
        try {
          ...
          const boxNameBytes = new Uint8Array(atob(box.name).split('').map(c => c.charCodeAt(0)));
          
          // Decode the ARC-4 structured box name
          const decodedBoxName = decodeBoxName(boxNameBytes);
          
          if (decodedBoxName && decodedBoxName.isValid && decodedBoxName.poolId === poolId) {
            console.log(`Found matching box for pool ${poolId}: ${decodedBoxName.userAddress}`);
            ...
            if (boxData && boxData.value) {
              ...
              const boxValueBytes = new Uint8Array(atob(boxData.value).split('').map(c => c.charCodeAt(0)));
      ...
      const poolMetrics = this.calculatePoolMetricsFromStakes(poolId, poolBoxes);
  ...
  // Calculate pool metrics from real stake data
  calculatePoolMetricsFromStakes(poolId, stakes) {
    ...
    stakes.forEach(({ stakeData }) => {
      ...
      if (stakeData.rewardType === 'APY' && stakeData.rewardRate > 0) {
        ...
        if (rateBP > 0 && amountRaw > 0 && timeStaked > 0) {
          const reward = Math.floor((amountRaw * rateBP * timeStaked) / (10000 * YEAR_SECONDS));
          totalRewards += reward;
        }
      }
    });
    ...
    return {
      ...
      totalTransactions,
      averageStake: uniqueStakers.size > 0 ? totalValue / uniqueStakers.size : 0,
      ...
      stakes
        ...
        .forEach(({ stakeData }) => {
          ...
          if (dailyData[dateStr]) {
            ...
            if (stakeData.rewardType === 'APY' && stakeData.rewardRate > 0) {
              const dailyReward = (stakeData.amount * stakeData.rewardRate / 10000) / 365;
  ...
  async getTopStakers(poolId, limit = 50) {
    try {
      ...
      const stakers = stakes.map((stake, index) => ({
        id: index + 1,
        ...
        joinDate: new Date(stake.stakeData.stakedAt * 1000).toISOString().split('T')[0],
  ...
  calculateRewards(stakeData) {
    ...
    if (rateBP > 0 && amountRaw > 0 && timeStaked > 0) {
      return Math.floor((amountRaw * rateBP * timeStaked) / (10000 * YEAR_SECONDS));
    }
